...
def join_room(message):
    ...
    if message.text == "Back 🔙":
        ...
        if message.text in data["rooms"]:
            ...
            save_data()
            bot.send_message(message.chat.id,'Room joined! Wait for the other player to start the game.',reply_markup=keyboard_back())
            ...
            bot.send_message(message.chat.id,'Such room does not exist!',reply_markup=keyboard_main())
            return
    else:
        bot.send_message(message.chat.id,'Wrong input!',reply_markup=keyboard_main())
        return
...
def get_score(message, room_code, chat_id):
    ...
    if message.text in ["Back 🔙", "Create Room 🏠", "Join Room 🔑"]: 
        bot.send_message(message.chat.id,"You are already in a game! Please continue playing. Try again:")
        bot.register_next_step_handler_by_chat_id(message.chat,id, get_score, room_code, chat_id)
    if not message.text.isdigit():
        bot.send_message(message.chat.id,'Wrong input! Try again (from -10 to 10):')
        bot.register_next_step_handler_by_chat_id(message.chat,id, get_score, room_code, chat_id)
        return
    ...
    x = data["rooms"][room_code]["players"][:]
    x.remove(chat_id)
    bot.send_message(chat_id,f'You scored: *{score}* points! Correct answer: *{value}*.\nTotal score: *{data["rooms"][room_code]["score"]}*', parse_mode='Markdown')
    ...
    if len(data["rooms"][room_code]["cards"]) > 0:
        ...
        bot.send_message(data["rooms"][room_code]["players"][0],f'The end! Your score: {data["rooms"][room_code]["score"]}',reply_markup=keyboard_back())
...
def ask_guess(message, chat_id, room_code, card):
    ...
    bot.send_message(message.chat.id,"Thanks! Please, wait for your teammate's guess.")
    bot.send_message(chat_id,f'Your teammate gave a clue for {card}: {message.text}.\nPlease guess the value from -10 to 10:',reply_markup=types.ReplyKeyboardRemove())
